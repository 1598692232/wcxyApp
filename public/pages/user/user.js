...
Page({
  ...
  onReady: function () {  
    let self = this
    let oneRad = self.data.storage_jindu/100
    let twoRad = self.data.project_jindu=='无限'?1:self.data.project_jindu/100
    let threeRad = self.data.member_jindu=='无限'?1:self.data.member_jindu/100
    // 页面渲染完成  
